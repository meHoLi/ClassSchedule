// pages/punchTheClock/punchTheClock.js
const app = getApp()
...
Page({
  ...
  setListData: function (query){
    ...
    wx.request({
      url: that.data.url + '/Clock/Index', //仅为示例，并非真实的接口地址
      data: {
      ...
      success: function (res) {
        let value = res.data.Data,
          haveDate = !value.todayClockList[0] && !value.mouthClockList[0] && !value.weekList[0] ? false : true,
        ...
        wx.getSystemInfo({
          success: function (res) {
            ...
            bodyH = res.windowHeight - 10 - 28 - 70
  ...
  //限时打卡——去番茄钟
  limitPunchCard: function(e){
    let that = this,
      item = e.currentTarget.dataset.item,
      childrenID = that.data.childrenID;

    // wx.navigateTo({
    //   url: '../pomodoro/pomodoro?childrenID=' + childrenID
    // })
  ...
  toIntegral: function(e){
    ...
    wx.navigateTo({
      url: '../bonusPointsList/bonusPointsList?childrenID=' + childrenID
    })
  },

  //编辑打卡任务
  editClock: function(e){
    let that = this,
      item = e.currentTarget.dataset.item,
      childrenID = that.data.childrenID;

    wx.navigateTo({
      url: '../setClock/setClock?childrenID=' + childrenID + '&id=' + item.ID
    })
  },
  ...
  addClock: function () {
    ...
    wx.navigateTo({
      url: '../clockProject/clockProject?childrenID=' + childrenID
    })
  },
  ...
  touchE: function (e) {
    if (e.changedTouches.length == 1) {
      ...
      var name = listType == 'day' ? 'todayClockList' : (listType == 'week' ? 'weekList' : 'mouthClockList')
